...
class CULane(BaseDataset):
    def __init__(self, data_root, split, processes=None, cfg=None, ori_img_h=590, ori_img_w=1640, cut_height=270):
        super().__init__(data_root, split, processes=processes, cfg=cfg)
        self.list_path = osp.join(data_root, LIST_FILE[split])
        self.split = split
        self.orig_img_h = ori_img_h
        self.orig_img_w = ori_img_w
    ...
    def get_prediction_string(self, pred):
        ...
        ys = np.arange(270, 590, 8) / self.orig_img_h
        out = []
        for lane in pred:
            xs = lane(ys)
            valid_mask = (xs >= 0) & (xs < 1)
            # xs = xs * self.cfg.ori_img_w
            xs = xs * self.orig_img_w
            lane_xs = xs[valid_mask]
            # lane_ys = ys[valid_mask] * self.cfg.ori_img_h
            lane_ys = ys[valid_mask] * self.orig_img_h
